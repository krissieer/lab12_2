...
namespace lab12_2
{
    public class MyHashTable<T> where T : IInit, ICloneable, new()
    {
        ...
        public int SearchItem(Point<T> item)
        {
            if (table == null)
            {
                return 0;
            ...
            else
            {
                if (item != null)
                {
                    ...
                    if (table[index].Data.Equals(item.Data)) //элемент найден
                        return 1;
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public int RemoveItem(Point<T> item)
        {
            if (table == null)
            {
                return 0;
            ...
            else
            {
                if (item != null)
                {
                    Point<T>? curr;
                    int index = GetIndex(item.Data);
                    if (table[index].Data.Equals(item.Data)) // если первый элемент в цепочке - искомый
                    {
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public MyHashTable<T> Clone(MyHashTable<T> hashTable)
        {
